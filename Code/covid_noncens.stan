functions {
  real viral_shape_one(real t, real log_r, real log_d, real m, real log_kappa) {
    ...
    // Updated manuscript V(t):
    // V(t) = exp(r*t) * {1 + (r/d)*exp[kappa*(t-m)]}^{-(r+d)/kappa}
    // This parameterisation makes m the exact peak time.
...
data {
  ...
  int<lower=1> N_obs;
  array[N_obs] int<lower=1> obs_index_pt;
  ...
  // Sparse replacement for ip2_idx[N_delta, N_ip].
  ...
  // Precomputed Park correction weight exp(-r_growth * ip_grid[ii]).
...
model {
...

  {
    ...
    for (jz in 1:N_z) {
      ...
      // Observed-pair transmission timing:
      // U is the selected observed transmission event, not the first event.
      // With p_trans(t) proportional to V(t), multiplicative scales such as
      // eta and exp(z) cancel after conditioning on an observed transmission.
      vector[N_u] rawU;
      vector[N_u] normU;
    ...
    for (ks in 1:N_norm) {
      ...
      for (kg in 1:N_gt) {
        ...
        for (jz in 1:N_z) {
          acc_s += z_std_w[jz] * fG_mass_z[kg, jz] * C_delta_z[dd, jz];
        }
      }
...
generated quantities {
...

  {
    ...
    for (n in 1:N_obs) {
      log_lik[n] = log_unnorm_si[obs_index_pt[n]] - log_norm;
    }

    for (n in 1:N_si) {
      log_lik_grouped[n] = counts[n] * (log_unnorm_si[obs_index[n]] - log_norm);
    }
    ...
    generation_prob_grid = fG_mix_mass;
