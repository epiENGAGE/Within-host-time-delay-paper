...
data {
  ...
  real<lower=0> dx_si;
  array[N_norm] int<lower=0> counts_full;
  int<lower=0> n_total;
...
model {
...

  {
    ...
    for (jl in 1:N_l) {
      ...
      for (ju in 1:N_u) {
        rawU[ju] = base_t[lu_t_idx[jl, ju]];
      }

      normU = normalize_density(rawU, dx_u);

      for (ju in 1:N_u) {
        normU_mat[jl, ju] = normU[ju];
      }
    }
    ...
    for (jz in 1:N_z) {
      ...
      for (jl in 1:N_l) {
        for (ju in 1:N_u) {
          int kg = gt_idx[jl, ju];
          fG_mass_z[kg, jz] += normL[jl] * normU_mat[jl, ju] * dx_lu;
        }
      }
    ...
    for (ks in 1:N_norm) {
      if (counts_full[ks] > 0) {
        target += counts_full[ks] * (log_unnorm_si[ks] - log_norm);
      }
    }
  }
}
...
generated quantities {
  ...
  real sd_gen;
  vector[N_norm] p_si;
  vector[N_norm] log_lik_cell;
  array[N_norm] int counts_rep;

  {
    ...
    // FAST CACHE for generated quantities too.
    for (jl in 1:N_l) {
      vector[N_u] rawU;
      vector[N_u] normU;
    ...
    for (ks in 1:N_norm) {
      ...
      unnorm_si[ks] = fmax(acc_s, 1e-300);
    }

    si_norm = sum(unnorm_si);
    p_si = unnorm_si / si_norm;

    for (ks in 1:N_norm) {
      log_lik_cell[ks] = counts_full[ks] * log(p_si[ks]);
    }

    counts_rep = multinomial_rng(p_si, n_total);
  }
}
